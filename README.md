...
- 트랜잭션 어노테이션과 커스텀 락 어노테이션의 순서를 정해줘야합니다.
    - 락 어노테이션이 트랜잭션 어노테이션을 감싸게끔 클래스를 분리해 해결
- 멀티 스레드 환경에서 락을 풀 때 스레드를 구분하지 않으면 정합성 문제가 생길 수 있습니다.
    - UUID를 발급해 value에 넣고 검사해 해결
- 스핀락 사용 시 타임아웃시간을 짧게 설정하면 락 획득에 실패하는 케이스가 늘어나는 걸 볼 수 있었습니다.

### 비관적 락 구현 시 발생한 이슈

- 설정된 `maximum pool size`(기본값 10) 이상의 스레드가 접근하면 커넥션 풀에 자리가 없어 누수가 발생했습니다.

### 낙관적 락 구현 시 발생한 이슈

- 동시에 같은 스레드가 낙관적 락에 접근할 시 한 개의 요청만 적용되고, 나머지는 데드락 발생했습니다.

### Redisson 구현 시 느낀 장점

- 레디스를 이용한 여러 유틸들이 미리 구현돼있다.
- 락 검증을 pub/sub 방식으로 검사하기 때문에 성능이 좋다.
...
[동시성 제어 프로젝트 정리](https://www.notion.so/27e2dc3ef514800a9baaf61b5e1dbb56?pvs=21) (상세 자료)
